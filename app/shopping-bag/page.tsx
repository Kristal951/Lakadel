...
import { Trash2, Plus, Minus, ArrowRight, Heart, X, ArrowLeft, ShieldCheck } from "lucide-react";
...
export default function ShoppingBag() {
  ...
  const totalPrice = cartItems.reduce((sum, i) => sum + i.quantity * i.product.price, 0);
    ...
    return (
      <section className="min-h-[70vh] flex flex-col items-center justify-center p-6 bg-background">
        <div className="bg-foreground/20 p-16 rounded-[3rem] border border-foreground flex flex-col items-center text-center max-w-md">
          ...
          <h2 className="text-3xl font-bold text-foreground">Your bag is empty</h2>
          <p className="text-foreground/70 mt-3 mb-8">
            Discovery awaits! Explore our latest arrivals and find something you'll love.
          </p>
          <button 
            onClick={goToShop}
  ...
  return (
    <section className="max-w-7xl mx-auto py-12 px-6 lg:px-12 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-12">
        <div>
          <h1 className="text-5xl font-extrabold tracking-tight text-foreground">Bag</h1>
        ...
        {!showClearConfirm ? (
          ...
          <div className="flex items-center gap-3 animate-in slide-in-from-right-4">
            ...
            <button onClick={() => { clearCart(); setShowClearConfirm(false); }} className="text-xs bg-red-500 text-white px-3 py-1.5 rounded-full">Yes, clear</button>
            <button onClick={() => setShowClearConfirm(false)} className="text-xs bg-neutral-200 px-3 py-1.5 rounded-full"><X size={12} /></button>
          </div>
        )}
      </div>
            ...
            >
              ...
              <div className="flex-1 flex flex-col py-2">
                <div className="flex justify-between items-start mb-1">
                  <h3 className="text-2xl font-bold text-foreground leading-tight">{item.product.name}</h3>
                  <p className="text-2xl font-semibold">₦{item.product.price.toLocaleString()}</p>
                </div>
                ...
                <div className="flex flex-wrap gap-x-6 gap-y-2 mt-2 text-sm uppercase tracking-wider font-semibold text-foreground/60">
                  ...
                  {item.selectedColor && <span>Color: <span className="text-foreground">{item.selectedColor}</span></span>}
                </div>
        ...
        <aside className="lg:col-span-4 space-y-6 sticky top-24">
          <div className="bg-foreground text-background p-8 rounded-[2.5rem]">
            ...
            <div className="space-y-5 mb-8">
              <div className="flex justify-between text-background/60 font-medium">
                <span>Subtotal</span>
                <span className="text-background font-semibold">₦{totalPrice.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-background/60 font-medium">
                <span>Shipping</span>
              ...
              <div className="pt-4 flex gap-2">
                ...
                />
                <button className="bg-background px-4 py-2 rounded-xl text-xs text-foreground font-bold uppercase hover:bg-foreground hover:text-background transition-colors">Apply</button>
              </div>
              ...
              <div className="pt-6 border-t border-background flex justify-between items-end">
                ...
                <span className="text-3xl font-bold tracking-tighter text-background">₦{totalPrice.toLocaleString()}</span>
              </div>
            </div>

            <button className="w-full bg-white text-black py-5 rounded-3xl font-bold flex items-center justify-center gap-2 hover:bg-neutral-100 transition-all duration-300 group active:scale-[0.98]">
              Proceed to Checkout
          ...
          <div className="bg-neutral-50 p-6 rounded-4xl border border-neutral-100">
            <div className="flex items-center gap-4 text-neutral-600 mb-4">
              ...
              <div>
                <p className="text-sm font-bold text-neutral-900">Secure Checkout</p>
