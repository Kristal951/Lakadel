  ...
  return (
    <div className="min-h-screen bg-background p-8 space-y-10">
      ...
      <header className="flex flex-col gap-1">
        ...
        <p className="text-sm text-foreground/50">Monitor customer behavior and lifetime value.</p>
      </header>

      {/* Customer Metrics Ribbon */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-0">
        <StatCard title="Total Customers" value="8,432" icon={Users} trend="+10%" />
        <StatCard title="New This Month" value="412" icon={UserPlus} trend="+24%" />
        <StatCard title="Active Now" value="89" icon={UserCheck} />
        <StatCard title="Avg. LTV" value="$542.50" icon={TrendingUp} trend="+3.2%" />
      </div>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-foreground/5 bg-foreground/1">
              ...
              <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-foreground/70">Last Order</th>
          ...
          <tbody className="divide-y divide-foreground/5">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <tr key={i} className="group hover:bg-foreground/1 transition-colors">
                <td className="px-8 py-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-foreground/40">jane.doe@example.com</span>
                    </div>
                  </div>
                </td>
                <td className="px-8 py-6">
                ...
                <td className="px-8 py-6 text-sm font-mono font-bold text-foreground">$1,240.00</td>
                <td className="px-8 py-6 text-sm text-foreground/60 italic">2 days ago</td>
