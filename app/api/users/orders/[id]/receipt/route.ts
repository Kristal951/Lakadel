...
export async function GET(
  ...
  try {
    ...
    if (order.status !== "PAID") {
      return NextResponse.json({ error: "Receipt available only after payment." }, { status: 400 });
    }
  ...
  return formatted.replace("₦", "NGN");
    ...
    // 1. Brand Header
    page.drawText("LAKADEL", { x: marginX, y, size: 24, font: bold, color: rgb(0.69, 0.05, 0.05) }); // Your brand red
    page.drawText("OFFICIAL RECEIPT", { x: 400, y: 785, size: 10, font: bold, color: rgb(0.5, 0.5, 0.5) });
    y -= 40;

    // 2. Info Grid
    const drawInfo = (label: string, value: string, xOffset: number) => {
      page.drawText(label, { x: xOffset, y, size: 9, font: bold, color: rgb(0.5, 0.5, 0.5) });
      page.drawText(value, { x: xOffset, y: y - 14, size: 10, font });
    };

    drawInfo("ORDER ID", order.id.toUpperCase(), marginX);
    drawInfo("DATE", order.paidAt ? new Date(order.paidAt).toLocaleDateString() : "-", 250);
    drawInfo("CUSTOMER", order.customerEmail || "-", 400);
    
    y -= 50;
    page.drawLine({ start: { x: marginX, y }, end: { x: pageWidth, y }, thickness: 1, color: rgb(0.9, 0.9, 0.9) });
    y -= 30;
    ...
    for (const it of order.orderItems) {
      ...
      page.drawText(money(it.quantity * it.price), { x: 480, y, size: 10, font });

      if (y < 150) break; // Basic overflow protection
    }

    y -= 40;
    page.drawLine({ start: { x: 350, y }, end: { x: pageWidth, y }, thickness: 1, color: rgb(0.9, 0.9, 0.9) });
    y -= 25;

    // 5. Totals Section
    const drawTotalRow = (label: string, value: string, isBig = false) => {
      page.drawText(label, { x: 350, y, size: isBig ? 12 : 10, font: isBig ? bold : font });
      page.drawText(value, { x: 480, y, size: isBig ? 12 : 10, font: isBig ? bold : font });
      y -= 20;
    };
    ...
    // 6. Footer
    y = 60;
    page.drawText("Thank you for your purchase!", { x: marginX, y, size: 10, font: bold });
    page.drawText("This is an electronically generated receipt.", { x: marginX, y: y - 12, size: 8, font, color: rgb(0.5, 0.5, 0.5) });
