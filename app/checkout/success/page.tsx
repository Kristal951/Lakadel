...
const formatDateTime = (v?: string | null) => {
  if (!v) return "-";
  const d = new Date(v);
  return Number.isNaN(d.getTime()) ? "-" : d.toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
};
...
export default function SuccessPage() {
  ...
  const fetchOrder = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const url = token ? `/api/users/orders/${orderId}?t=${encodeURIComponent(token)}` : `/api/users/orders/${orderId}`;
  ...
  const theme = {
    PAID: { color: "emerald", icon: <CheckCircle2 className="w-8 h-8" />, label: "Paid" },
    FAILED: { color: "red", icon: <XCircle className="w-8 h-8" />, label: "Failed" },
    PENDING: { color: "amber", icon: <Clock3 className="w-8 h-8 animate-spin-slow" />, label: "Verifying" },
  }[status];
      ...
      <div className="w-full max-w-lg relative group">
        ...
        <div className="relative bg-background border border-foreground/50 rounded-[2.5rem] overflow-hidden">
          <div className="relative p-8 pt-12 text-center flex flex-col items-center">
             <div className={`mb-6 w-20 h-20 rounded-3xl flex items-center justify-center shadow-2xl rotate-3 group-hover:rotate-0 transition-transform duration-500 bg-${theme.color}-500 text-white`}>
                {theme.icon}
             </div>
             ...
             <div className="mt-6 flex items-center gap-2">
                <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border bg-${theme.color}-500/10 border-${theme.color}-500/20 text-${theme.color}-600`}>
                  {theme.label}
                </span>
                {order?.paymentMethod && (
          ...
          <div className="px-8 pb-10">
            ...
            <div className="mt-8 flex flex-col gap-3">
               <a
                ...
                className={`flex items-center justify-center gap-2 h-14 rounded-2xl font-bold text-sm transition-all ${isPaid ? 'bg-background text-foreground border hover:scale-[1.02] hover:bg-foreground hover:text-background active:scale-95' : 'bg-muted text-muted-foreground pointer-events-none'}`}
               >
